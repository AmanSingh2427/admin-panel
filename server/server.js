...
// Multer Setup for Image Uploads with custom folder
const storages = multer.diskStorage({
  destination: './custom_folder/', // Change the destination folder here
  filename: (req, file, cb) => {
    cb(null, `${file.fieldname}-${Date.now()}${path.extname(file.originalname)}`);
  },
});
...
const uploads = multer({
  storage: storages,
  limits: { fileSize: 1000000 }, // Limit image file size to 1MB
  fileFilter: (req, file, cb) => {
    checkFileType(file, cb);
  },
}).single('photo'); // Assuming 'photo' is the name attribute of the file input in your frontend form
...
app.put('/products/:id', upload, async (req, res) => {
  const productId = req.params.id;
  const { name, price, description } = req.body;
  ...
  try {
    const query = `
      UPDATE aman.products 
      SET name = $1, price = $2, description = $3, photo = COALESCE($4, photo)
...
app.delete('/products/:id', async (req, res) => {
  const productId = req.params.id;
  try {
    const query = 'DELETE FROM aman.products WHERE id = $1 RETURNING *';
    const { rows } = await pool.query(query, [productId]);
    if (rows.length === 0) {
      return res.status(404).json({ message: 'Product not found' });
    }
